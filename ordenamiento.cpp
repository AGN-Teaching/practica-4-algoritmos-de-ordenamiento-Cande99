...
void mostrar_tabla(int num_arreglos, vector<vector<double>> &tiemposAlgoritmos) {
    cout << "Repeticiones    | Insertion Sort | Selection Sort | Bubblesort | Merge Sort | Quicksort |" << endl;

    for (int i = 0; i < num_arreglos; i++) {
        cout << setw(16) << i + 1 << " | ";

        for (size_t j = 0; j < tiemposAlgoritmos.size(); j++) {
            cout << fixed << setw(15) << setprecision(4) << tiemposAlgoritmos[j][i] << " | ";
        }
...
int main(int argc, char *argv[]) {
    if (argc != 3) {
        ...
        mostrar_tabla(NUM_ARREGLOS, tiemposAlgoritmos);
